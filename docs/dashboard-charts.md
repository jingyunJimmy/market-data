...
| **Data quality** | none (findings are tabular by nature) | Findings · per-finding evidence · missing timestamps · intelligent insights, on request | `/quality/report`, `/quality/issue-details`, `/quality/missing-timestamps`, `POST /insights` |
...
date, so a Chicago evening is not split at midnight UTC.
...
**Intelligent insights** sit below the findings and the missing-timestamp
listing. Nothing is requested until *Generate insights* is pressed; the result
lists recurring patterns, each with the figures of the evidence rows it cites
and the rules it suggests. See [insights.md](insights.md#the-api-and-the-dashboard).
...
| *Generate insights* not pressed for the current selection | `insightsQuery` | nothing is generated by opening the tab or moving a filter; an LLM call happens only when asked |
